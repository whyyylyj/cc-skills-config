...
This is a configuration repository for Claude Code containing Skills, MCP servers, Commands, and Hooks that users can install via an interactive installer. The repository provides a centralized way to share and distribute Claude Code configurations.
...
- `mcp-server.json`: MCP server configuration with command, args, env
- Optional `install.sh`: Installation script for setup
- Example: MySQL MCP server with environment variable configuration
...
**Hooks** (`hooks/<name>/hook.sh`):
- Bash scripts executed at specific events
- Must be executable (`chmod +x`)
- Return 0 for success, non-zero to block action
- Example: `user-prompt-submit` hook validates prompts before submission

### Configuration Installation Paths

The installer copies configurations to:
- Skills: `~/.claude/skills/<name>/skill.md`
- MCP Servers: `~/.claude/mcp_servers/<name>/`
- Commands: `~/.claude/commands/<name>/command.md`
- Hooks: `~/.claude/hooks/<name>/hook.sh`
